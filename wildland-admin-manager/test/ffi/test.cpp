...
int main()
{
    ...
    Box<EmailClientMockBuilder> email_client_mock_builder = create_boxed_email_client_mock_builder();
    email_client_mock_builder->expect_send(::rust::String("test@email.com"), ::rust::String("123456"), 1);
    Box<DynEmailClient> email_client_mock = email_client_mock_builder->build_boxed();

    // Create admin manager filled with mocked email_client
    Box<AdminManager> admin_manager = create_admin_manager(email_client_mock);
