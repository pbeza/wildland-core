            ...
            .filter_map(
                |PathWithStorages {
                    ...
                    match storages {
                        ...
                        None => Some(Either::Left(std::iter::once(NodeDescriptor {
                            storage: None,
                            absolute_path: path.join(path_within_storage.file_name().unwrap()),
                        }))),
                        Some(storages) => {
                            let backends_with_storages =
                                self.assign_backends_to_storages(&storages);
